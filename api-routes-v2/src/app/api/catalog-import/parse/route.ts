  ...
  try {
    ...
    if (file) {
      ...
      if (file.type === 'text/plain' || file.type === 'text/csv') {
        ...
        documentContent = `[Contenido del archivo: ${file.name} - Se requeriria extraccion de texto para PDFs]`;
      }
    }
    ...
    const catalogImport = await db.catalogImport.create({
      data: {
        ...
        type: importType,
        fileName: fileName || 'Importacion de texto',
...
Cada producto debe tener al menos un nombre. Busca tambien referencias, codigos, precios, unidades y descripciones.
  ...
  "products": [
    {
      ...
      "reference": "codigo o referencia",
      "sku": "SKU si existe",
      "description": "descripcion del producto",
      "unit": "unidad de medida",
      "price": precio como numero o null,
      "category": "categoria si esta indicada"
    }
  ],
  "totalFound": numero total de productos,
  "currency": "moneda detectada",
  "confidence": "alto/medio/bajo"
}

Si no encuentras productos claros, devuelve un array vacio.
    ...
    try {
      const aiResponse = await zai.chat.completions.create({
        messages: [
          {
            role: 'system',
            content: `Eres un especialista en extraccion de datos de catalogos de productos para una empresa de construccion.
Tu tarea es identificar productos, sus referencias y precios de cualquier documento o lista.
Se exhaustivo y trata de encontrar todos los productos posibles.`,
          },
          {
            role: 'user',
    ...
    return NextResponse.json(
      { error: 'Error al procesar el catalogo' },
      { status: 500 }
    );
  }
}

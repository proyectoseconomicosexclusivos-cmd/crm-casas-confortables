...
// PUT /api/alerts/[id] - Marcar como leida/resuelta
...
export const POST = withRole(['SUPER_ADMIN', 'ADMIN'], async (req: NextRequest, session: any) => {
  ...
  if (!type || !title || !message) {
    return errorResponse('Tipo, titulo y mensaje son requeridos');
  }
...
// POST /api/alerts/check - Ejecutar verificacion de alertas
export const CHECK_ALERTS = withRole(['SUPER_ADMIN', 'ADMIN'], async (req: NextRequest, session: any) => {
  const alerts: any[] = [];

  // 1. Leads frios (sin contacto en X dias)
  ...
  for (const lead of coldLeads) {
    alerts.push({
      ...
      message: `El lead de ${lead.firstName} ${lead.lastName} lleva mas de 7 dias sin contacto`,
      referenceType: 'lead',
      referenceId: lead.id,
  ...
  for (const work of delayedWorks) {
    alerts.push({
      ...
      message: `La obra "${work.name}" deberia haber terminado el ${work.endDate?.toLocaleDateString()}`,
      referenceType: 'work',
      referenceId: work.id,
  ...
  for (const task of overdueTasks) {
    alerts.push({
      ...
      severity: 'INFO',
      title: 'Tarea vencida',
      message: `La tarea "${task.title}" esta vencida`,
      referenceType: 'task',
      referenceId: task.id,
  ...
  for (const alert of alerts) {
    // Verificar si ya existe una alerta similar reciente (ultimas 24h)
  ...
  }, `Verificacion completada: ${alerts.length} alertas generadas`);
});

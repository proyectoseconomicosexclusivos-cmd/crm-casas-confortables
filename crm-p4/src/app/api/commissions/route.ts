...
export const POST = withRole(['SUPER_ADMIN', 'ADMIN'], async (req: NextRequest, session: any) => {
  ...
  await db.notification.create({
    data: {
      ...
      message: `Se ha registrado una comisión de ${new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(parseFloat(amount))}`,
    },
  });

  return successResponse(commission, 'Comisión creada exitosamente');
});

// PUT /api/commissions/[id] - Actualizar estado de comisión
export const PUT = withAuth(async (req: NextRequest, session: any, params: any) => {
  ...
  if (!commission) {
    return errorResponse('Comisión no encontrada', 404);
  }
  ...
  return successResponse(updated, 'Comisión actualizada');
});

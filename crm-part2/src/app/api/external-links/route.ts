...
export const GET = withAuth(async (req: NextRequest, session: any) => {
  ...
  // Filtrar por visibilidad según rol del usuario
  ...
  const filteredLinks = links.filter(link => {
    ...
    // Si no es público y no tiene roles específicos, solo admins pueden verlo
...
export const POST = withRole(['SUPER_ADMIN', 'ADMIN', 'TEAM_LEADER'], async (req: NextRequest, session: any) => {
  ...
  if (!name || !url || !category) {
    return errorResponse('Nombre, URL y categoría son requeridos');
  }

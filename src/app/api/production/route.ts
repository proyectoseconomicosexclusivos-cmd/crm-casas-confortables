...
// GET /api/production - Listar pedidos de producción
...
// POST /api/production - Crear pedido de producción
export const POST = withRole(['SUPER_ADMIN', 'ADMIN', 'TEAM_LEADER'], async (req: NextRequest, session: any) => {
  ...
  if (!orderNumber || !name) {
    return errorResponse('Número de pedido y nombre son requeridos');
  }
  ...
  await db.productionTimeline.create({
    data: {
      ...
      event: 'created',
      description: 'Pedido de producción creado',
    },
  });

  return successResponse(order, 'Pedido de producción creado exitosamente');
});

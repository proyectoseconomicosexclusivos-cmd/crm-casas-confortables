  ...
  try {
    ...
    if (body.status) {
      ...
      if (!validTransitions[task.status]?.includes(body.status)) {
        return NextResponse.json(
          { error: `Transición de estado no válida: ${task.status} → ${body.status}` },
          { status: 400 }
        )
      }
    ...
    // Si se completó, verificar penalizaciones automáticas
    if (body.status === 'COMPLETED') {
      await applyAutomaticPenalties(id)
    }

    // Si se validó, actualizar ranking
...
export async function DELETE(
  ...
  try {
    ...
    // Solo se puede eliminar si está asignada

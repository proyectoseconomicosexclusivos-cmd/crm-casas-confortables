...
export async function GET(request: NextRequest) {
  try {
    ...
    // Obtener tareas pendientes de validación (estado COMPLETED)
...
// POST - Crear validación (aprobar/rechazar)
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validation.isValid && status === 'approved') {
      return NextResponse.json(
        { error: 'La tarea no cumple con los requisitos mínimos', details: validation },
        { status: 400 }
      )
    }
    ...
    // Si se aprobó, actualizar ranking
    if (status === 'approved') {
    ...
    return NextResponse.json(
      { error: 'Error al crear validación' },
      { status: 500 }
    )
  }
}
